 ...
 */
    ...
    public:
        ...
        struct ConstIterator {
            ...
            /*!
             * \brief Constructor. Creates ConstIterator pointing at a bit and holds
             * pointer to data.
             * \param data Pointer to the beginning of the \c BitVector.
             * \param position Position the iterator is pointing at.
             */
            ConstIterator(uint64_t* const data, size_t const position) noexcept
                    : bit_access_(data, position) {}

            /*!
             * \brief ConstIterator is dereferenceable. Obtain value it is pointing at.
             * \return Reference to the bit the iterator points at.
             */
            ...
            //! Postfix increment.
            ConstIterator operator++(int32_t) noexcept {
                auto tmp = *this;
                ++(*this);
                return tmp;
            }

            //! ConstIterator comparison equality.
            friend bool operator==(ConstIterator const& a, ConstIterator const& b) noexcept {
                return a.bit_access_ == b.bit_access_;
            }

            //! ConstIterator comparison inequality.
            friend bool operator!=(ConstIterator const& a, ConstIterator const& b) noexcept {
                return a.bit_access_ != b.bit_access_;
            }

            //! ConstIterator distance computation.
            friend difference_type operator-(ConstIterator const& a,
                                             ConstIterator const& b) noexcept {
                return a.bit_access_.distance(b.bit_access_);
            }

        private:
            BitAccess bit_access_;
        }; // struct BitVector::ConstIterator

        /*!
         * \brief Get const iterator representing the first element of the \c BitVector.
         * \return ConstIterator representing the first element of the \c BitVector.
         */
        ConstIterator begin() const noexcept {
            return ConstIterator(raw_data_, 0);
        }

        /*!
         * \brief Get const iterator representing the end of the \c BitVector.
         *
         * The \c end() method returns a const iterator that may refer to an invalid
         * memory address. It should never be accessed directly.
         * \return ConstIterator representing the end of the \c BitVector.
         */
        ConstIterator end() const noexcept {
            return ConstIterator(raw_data_, bit_size_);
        }
         ...
         * \brief Get const iterator representing the end of the \c BitVector.
         *
         * The \c cend() method returns a const iterator that may refer to an invalid
         * memory address. It should never be accessed directly.
         * \return ConstIterator representing the end of the \c BitVector.
         */
